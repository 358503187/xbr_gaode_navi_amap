...
class GaodeLocation {
  ...
  static const MethodChannel _methodChannel = XbrGaodeNaviAmap.channel;
  static const EventChannel _eventChannel = EventChannel("xbr_gaode_location_stream");
  static const String xbrGaodeBackgroundLocationKey = "xbr_gaode_background_location";
  ...
  Future<AMapAccuracyAuthorization> getSystemAccuracyAuthorization() async {
    int result = -1;
    if (Platform.isIOS) result = await _methodChannel.invokeMethod("getSystemAccuracyAuthorization", {'pluginKey': _pluginKey});
    if (result == 0) {
  ...
  ///定位结果回调  => 定位结果以map的形式透出，其中包含的key已经含义如下：
